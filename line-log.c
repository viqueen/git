 ...
 * praise for all the + (target-side) ranges.  So for every pair of
				       ...
				       struct line_log_data *range)
{
	...
	for (i = 0; i < nparents; i++) {
		...
		if (!changed) {
			/*
			 * This parent can take all the praise, so we
	...
	/*
	 * No single parent took the praise.  We add the candidates
